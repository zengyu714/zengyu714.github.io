...
投影 $ \boldsymbol{p} $ 在线 $ \boldsymbol{a} $ 上，是 $ \boldsymbol{a} $ 的一部分，能表示成 $ \boldsymbol{p} = \hat{x} \boldsymbol{a} $，某个比例 $ \hat{x} $ 乘上 $ \boldsymbol{a} $。如果求得了 $ \hat{x} $，就有投影 $ \boldsymbol{p} $，再根据 $ \boldsymbol{p} = P \boldsymbol{b} $ 得到投影矩阵 $ P $。
...
3. 最后求出投影矩阵 $ P $，在投影向量 $ \boldsymbol{p} $ 的公式中，可以找出 $ \boldsymbol{p} = P \boldsymbol{b} $ 的形式吗？
    ...
    $ P $ 是一个列向量 $ \boldsymbol{a} $ 乘行向量 $ \boldsymbol{a}^T $，再除一个数 $ \boldsymbol{a}^T\boldsymbol{a} $。已经知道 $ \boldsymbol{a} $ 是 $ m \times 1 $ 的列向量，所以 $ P $ 是一个 $ m \times m $ 的投影矩阵，**秩为1**。我们将向量投进一维的子空间，是沿着 $ \boldsymbol{a} $ 的线，这条线在 $ P $ 的列空间内。
...
当 $ n = 1 $ 时，就是上一小节投影在线上的情况，$ \boldsymbol{a}_1 $ 是 $ A $ 仅有的一列。现在的 $ A $ 有 $ n $ 列，$ \boldsymbol{a}_1,\ \cdots ,\ \boldsymbol{a}_n $。特殊的组合 $ \hat{\boldsymbol{x}} = (\hat{x}_1,\ \cdots ,\ \hat{x}_n) $ 使得投影后的向量 $ \boldsymbol{p} = A \hat{\boldsymbol{x}} $ 与 $ \boldsymbol{b} $ 最接近，$ \hat{\boldsymbol{x}} $ 头上的帽子表示这是一个最优组合（best choice）。

计算投影到 $ n $ 维子空间的方法与之前一样：① 找到 $ \hat{\boldsymbol{x}} $（变成向量）② 找到 $ \boldsymbol{p} $ ③ 找到 $ P $。
...
如果 $ A \boldsymbol{x} = \boldsymbol{b} $ 无解，就左乘上 $ A^T $ 然后解 $ A^TA \hat{\boldsymbol{x}} = A^T \boldsymbol{b} $。
...
先给出近似解 $ \hat{\boldsymbol{x}} = (5,\ -3) $，即直线 $ 5 - 3t $ 是最接近三个点的。
...
2. 代数
    ...
    每一个 $ \boldsymbol{b} $ 都可以分成垂直的两部分，$ \mathbb{C}(A) $ 中的 $ \boldsymbol{p} $ 和与 $ \mathbb{N}(A^T) $ 正交的 $ \boldsymbol{e} $。因为 $ \boldsymbol{p} $ 在列空间中，所以 $ A \hat{\boldsymbol{x}} = \boldsymbol{p} $ 是一定可解的，这就是把 $ \boldsymbol{e} $ 暂时丢掉，解 $ A^TA \hat{\boldsymbol{x}} = A^T \boldsymbol{b} $。
    ...
    所以对于任意 $ \boldsymbol{x} $，从长度的平方看都有 $ \lVert A\boldsymbol{x} - \boldsymbol{b} \rVert^2 = \lVert A\boldsymbol{x} - \boldsymbol{p} \rVert^2 + \lVert \boldsymbol{e} \rVert^2 $。令 $ \boldsymbol{x} = \hat{\boldsymbol{x}} $，这样等式右边的 $ \lVert A\boldsymbol{x} - \boldsymbol{p} \rVert^2 $ 就缩减至零，再加上最小误差 $ \boldsymbol{e} = (e_1,\ e_2,\ e_3) $（没法再小了）。
...
举个例子，假设在 $ t = -2,\ 0,\ 2 $ 时 $ b = 1,\ 2,\ 4 $，常数项 $ C $ 的三个系数 $ (1,\ 1,\ 1) $ 与 $ (-2,\ 0,\ 2) $ 正交。
...
**正方形**的 $ Q $ 有个名字：**正交矩阵（orthogonal matrix）**，其它列向量标准正交的**矩**阵用字母 $ Q $ 表示就ok了。
...
1. 旋转（rotation）正交矩阵
    ...
    这两点也说明了 $ col_1,\ col_2 $ 是 $ \mathbb{R}^2 $ 内的标准正交基。
...
- 相对角度不变，$ (Q \boldsymbol{x})^T(Q \boldsymbol{y}) = \boldsymbol{x}^T \boldsymbol{y} $
    ...
    角度可以用内积表示为 $ cos\theta = \cfrac{\boldsymbol{x}^T \boldsymbol{y}}{\lVert \boldsymbol{x} \rVert \lVert \boldsymbol{y} \rVert} $，所以角度不变。
...
上一节的第一步是 $ \boldsymbol{q}_1 = \boldsymbol{a}/\lVert \boldsymbol{a} \rVert $，没有涉及到后面的向量 $ \boldsymbol{b},\ \boldsymbol{c} $，而 $ \boldsymbol{b} $ 又是 $ \boldsymbol{A},\ \boldsymbol{B} $ 的组合，与 $ \boldsymbol{C} $ 无关。
...
2. $ A^TA \hat{\boldsymbol{x}} = A^T \boldsymbol{b} \Rightarrow R^TR \hat{\boldsymbol{x}} = R^TQ^T \boldsymbol{b} \Rightarrow R \hat{\boldsymbol{x}} = Q^T \boldsymbol{b} $
    ...
    注意 $ R $ 是一个上三角方阵，所以 $ \hat{\boldsymbol{x}} $ 可以用后向代换（back substitution）解出。


### 快速练习

**Q1**.
- $ A_{m \times 3} $ 有三个正交的列向量，长度都是 $ 4 $，$ A^TA $ 是【$ 16I_{m \times m} $】。
- $ A_{m \times 3} $ 有三个正交的列向量，长度是 $ 1,\ 2,\ 3 $，$ A^TA $ 是【对角矩阵，对角线上前三个元素是 $ 1,\ 4,\ 9 $。
