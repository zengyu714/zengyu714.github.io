...
3. **特解($A\boldsymbol{y} = b$) + 通解($A\boldsymbol{z} = 0$)**：整个向量空间的解 $\boldsymbol{x} = \boldsymbol{y} + \boldsymbol{z}$ 就有了，如[封面图](http://math.mit.edu/~gs/linearalgebra/linearalgebra5_Front.jpg)所示。其中通解也称**零空间(Nullspace)**的解。
